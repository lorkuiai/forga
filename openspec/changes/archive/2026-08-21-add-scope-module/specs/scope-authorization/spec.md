...
#### Scenario: Allowed permission in active scope

- **WHEN** the subject has the required permission for the resource under the active scope
...
- **WHEN** the subject has permission in one scope but the request is evaluated under another active scope
...
The SDK SHALL provide a way to create query constraints that bind protected queries to the active scope.
